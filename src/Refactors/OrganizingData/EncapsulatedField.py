...
def EncapsulatedField(version1_name, version2_name, block, file1, file2, name_1, name_2):
    refactors = []
            ...
            if f1_init_function_object is not None:
                for line_f2 in file2.Lines:
                    if line_f2.line_type.name == "Class":
                        if line_f2.line_type.name_of_the_class == f1_line.line_type.name_of_the_class:
                                ...
                                for f1_atr in f1_class_atr:
                                    ...
                                    for _f2_atr in _f2_class_atr:
                                        ...
                                        if "_"+f1_atr_name == _f2_atr_name:
                                            ...
                                            if not decl_content_f1.endswith("]" or ")" or "}") and not decl_content_f2.endswith("]" or ")" or "}"):
                                                ...
                                                if (any("get" in method.function_name for method in methods_of_f2_class) or \
                                                    any("Get" in method.function_name for method in methods_of_f2_class) or \
                                                    any("set" in method.function_name for method in methods_of_f2_class) or \
                                                    any("Set" in method.function_name for method in methods_of_f2_class)) and \
                                                    any(f1_atr_name.lower() in method.function_name.lower() for method in methods_of_f2_class):
                                                        refactors.append(Refactor("Encapsulated Field",
                                                                            name_1,
                                                                            name_2,
                                                                            version1_name,
                                                                            version2_name, 
